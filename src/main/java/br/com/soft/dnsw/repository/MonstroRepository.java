package br.com.soft.dnsw.repository;

import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;

import br.com.soft.dnsw.model.Monstro;
import br.com.soft.dnsw.repository.querys.MonstroQuery;

public interface MonstroRepository extends JpaRepository<Monstro, Integer> {

	@Query(value=MonstroQuery.FIND_BY_EXAMPLE, nativeQuery=true)
	List<Monstro> findByExample(@Param("nome") String nome, 
								 @Param("descricao") String descricao, 
								 @Param("idSistema") Integer idSistema, 
								 @Param("flagAtivo") Boolean flagAtivo);
}
