package br.com.soft.dnsw.repository;

import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;

import br.com.soft.dnsw.model.Vantagem;
import br.com.soft.dnsw.repository.querys.VantagemQuery;

public interface VantagemRepository extends JpaRepository<Vantagem, Integer> {

	@Query(value=VantagemQuery.FIND_BY_EXAMPLE, nativeQuery=true)
	List<Vantagem> findByExample(@Param("nome") String nome, 
								 @Param("descricao") String descricao, 
								 @Param("idSistema") Integer idSistema, 
								 @Param("flagAtivo") Boolean flagAtivo);
}
